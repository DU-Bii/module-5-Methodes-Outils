...
Il s'agit de sélectionner les informations du gène ENSG00000091831 à partir du fichier d'annotation puis d'utiliser un outil de la suite bedtools qui permet d'extraire la séquence génomique d'éléments.
...
Quelles sont les deux options à rajouter ? Quel est donc ce fichier `genome` ?

Le fichier genome doit recenser sur deux colonnes le nom du chromosome et sa longueur. Cette information est disponible dans le fichier `.fai` qui doit accompagner le génome. C'est son index, qui est créé par la commande `samtools faidx`. Ce fichier est déjà disponible dans le répertoire `data`.


    head data/Homo_sapiens.GRCh38.dna.toplevel.fa.fai 
...
Le détail des colonnes est disponible <a href="http://www.htslib.org/doc/faidx.html">ici</a>.

Il suffit alors de sélectionner les deux premières colonnes et de rediriger la sortie dans un fichier : 

```{bash, eval=FALSE}
cut -f1,2 data/Homo_sapiens.GRCh38.dna.toplevel.fa.fai > hg38.genome
```


```{bash, eval=FALSE}
srun bedtools intersect -b ENSG00000091831_gene.gff3 -a data/common_all_20180418.vcf -sorted -g > hg38.genome
```

Voici la démonstration de l'efficacité de cet algorithme :


    date ; srun bedtools intersect -b ENSG00000091831_gene.gff3 -a data/common_all_20180418.vcf -header -sorted -g data/my.genome > ENSG00000091831_known.vcf2 ; date
...
*Étape 3*: Rediriger la sortie dans un ficher VCF *valide* (avec le header)

Quelle option rajouter ?

```{bash, eval=FALSE}
srun bedtools intersect -b ENSG00000091831_gene.gff3 -a data/common_all_20180418.vcf -header > ENSG00000091831_known.vcf
```

*Étape 4*: On compte le nombre de lignes de variants
...
Il s'agit dans cet exercice d'utiliser les fichiers `GTF` contenant pour chaque échantillon les coordonnées des transcrits détectés dans cette étude.
Attention, l'annotation des fichiers `GTF` n'est pas conforme avec nos fichiers !
...
Il s'agit ici d'utiliser les fichiers `BAM` qui contiennent les reads mappés sur génome humain qui sont présents dans le répertoire `data`.
...
### Bonus : Combien de SNVs (single nucleotide variants) sont communs et différents entre TNBC1 et NonTNBC1 sur le gène ?

Vous aurez besoin, en plus des outils déjà vus dans ce TP, de l'outil <strong class="tool">vcftools</strong> pour ne garder que les SNVs du fichier VCF (qui contient également des INDELs).
...
```{bash, eval=FALSE}
vcftools --vcf ENSG00000091831_known.vcf --remove-indels --recode --recode-INFO-all --stdout > ENSG00000091831_known_SNV.vcf
vcftools --vcf data/TNBC1.vcf --remove-indels --recode --recode-INFO-all --stdout > TNBC1_SNV.vcf
vcftools --vcf data/NonTNBC1.vcf --remove-indels --recode --recode-INFO-all --stdout > NonTNBC1_SNV.vcf
...
# Spécifiques à NonTNBC1 et non connus ?
