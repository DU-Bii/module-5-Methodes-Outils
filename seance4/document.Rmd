...
*Étape 1*: Extrayez la région de 2 kb en amont de `ENSG00000091831`.

```{bash, eval=FALSE}
# Il me semble que l'on n'utilise finalement pas cette ligne...
...
*Étape 2*: Quels sont les variants connus situés dans cette région?
Comptez-les.
...
*Étape 0*: Comparer la façon de nommer les chromosomes dans les différents fichiers d'annotation (transcrits trouvés dans l'analyse, et annotation de référence).
...
Transformez-les en fichiers en fichier BED.
Triez-les.
Faites ceci en 2 commandes uniquement, une pour chaque fichier.

*Attention: les fichiers BED sont 0-based, pas les GTF.  Il y a donc une petite erreur ici !*
...
## Quel est le miARN le plus proche de ce gène ?
